...
class TIFFData(Adapter):
    ...
    def get(self, field: str, **kwargs) -> bytes | None:
        ...
        return self.image.get(key=field, **kwargs)

    def set(self, field: Field | str, value: bytes, **kwargs) -> Adapter:
        if isinstance(field, Field):
            field = field.name
        elif isinstance(field, str):
            pass
                ...
                if isinstance(encoded := model.encode(order=order, **options), bytes):
                    for fieldname, cläss in self.mapping.items():
                        if isinstance(model, cläss):
                            self.set(field=fieldname, value=encoded)
                            break
